...
class TuyaSHOpenAPI {
  ...
  async getACStatus(deviceID) {
    let res = await this.get(`/v2.0/cloud/thing/${deviceID}/shadow/properties`);
    return res.result;
  }
  ...
  async sendCommand(deviceID, params) {
    if (
      params.commands[0].code === "cool_wind_speed" ||
      params.commands[0].code === "child_lock" ||
      params.commands[0].code === "switch_vertical" ||
      params.commands[0].code === "mode"
    ) {
      const properties = {};
      params.commands.forEach((command) => {
        properties[command.code] = command.value;

        if (params.commands[0].code === "cool_wind_speed") {
          properties["fan_wind_speed"] = command.value;
        }

        if (params.commands[0].code === "switch_vertical") {
          properties["switch_horizontal"] = command.value;
        }

        if (params.commands[0].code === "mode") {
