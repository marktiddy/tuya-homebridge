...
class DuctlessAirConditionerAccessory extends BaseAccessory {
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      ...
      if (statusMap.hasOwnProperty("102")) {
        const value = statusMap["102"];
        let speed = 100;
        // Strong / High / Gentle / Sleep
        switch (value) {
          case "High":
            speed = 75;
            break;
  ...
  getSendParam(name, hbValue, service = "") {
    ...
    switch (name) {
      ...
      case Characteristic.RotationSpeed:
        ...
        if (currentSpeedValue >= 75) {
          speed = "Strong";
        } else if (currentSpeedValue > 50 && currentSpeedValue < 75) {
          speed = "High";
        } else if (currentSpeedValue >= 25) {
          speed = "Gentle";
        } else {
          speed = "Sleep";
        }
